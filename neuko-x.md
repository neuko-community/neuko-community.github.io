# Neuko X Transmissions

> Complete catalog of every Neuko X post/video with detailed breakdowns, puzzle clues, lore elements, and community reactions.

## Post Catalog

### Post #1: First Teaser
**Date**: October 11, 2025  
**Post ID**: `1976994494383112304`  
**Caption**: "i stood up i fell over i wept on the floor"  
**Media**: None (text-only reply)  
**Engagement**: Early teaser, establishes cryptic poetic tone  
**Significance**: First public signal from Neuko account before official launch  
**Lore Elements**: Establishes cryptic, emotional tone  
**Source**: `sources/info-dump.md`

---

### Post #2: "We're Watching" - Official Launch
**Date**: October 13, 2025, 8:48 AM UTC  
**Post ID**: `1977763395408429229`  
**Caption**: "we're watching"  
**Video Duration**: ~10 seconds  
**Video Description**: 
- Red/yellow logo fades in/out
- Hooded red-caped figure (G*Boy) in dark lab with monitors
- Figure interacts with equipment, smashes vending machine
- Walks dimly lit hallway toward light
- Audio: "You gotta tell us what happens when we get there"
- Style: Dark, suspenseful, cyberpunk vibes

**Engagement**: 
- 123K+ views organically
- Broke X algorithm without paid promotion
- Average 1K-3K likes, 200-700 reposts, 100K+ views
- Viral spread to timelines of people who usually only see DeFi content

**Puzzle/Lore Hooks**: 
- Establishes surveillance motif ("we're watching")
- Introduces G*Boy character (hooded figure)
- Lab setting suggests experiments/research facility
- Vending machine destruction hints at rebellion/chaos

**Community Reactions**: 
- Immediate interest and speculation about G*Boy
- People started asking questions and making posts
- Created initial hype and FOMO

**Significance**: Official launch, establishes core visual and narrative identity  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`

---

### Post #3: "Early Matters" - Badge Tease
**Date**: October 17, 2025  
**Post ID**: `1979268208340406384`  
**Caption**: "early matters"  
**Video Duration**: ~21 seconds  
**Video Description**:
- Panoramic white lab with cluttered desks, old computers, papers
- Zooms on details (notebooks, screens)
- Transitions to badges pinned on clothing
  - Rabbit 🐰 badges shown with clean cases
  - Moth 🐝 badges shown slightly worn/used
- Ends with hooded figure in rainy night alley
- Ambient sounds/song
- Style: Clean, exploratory, teasing rewards

**Founder Comments**: All three founders (@austin_hurwitz, @SQU1NCH, @greenie_sr) posted cryptic "🐰" comment

**Puzzle/Lore Hooks**: 
- Introduces Rabbit 🐰, Moth 🐝, and Snake 🐍 badge system
- Badge condition differences hint at distribution phases
- Lab setting continues facility theme
- "Early matters" suggests time-sensitive rewards

**Community Reactions**: 
- People felt FOMO seeing messages about 250 early rabbits
- Speculation about what badges mean
- Excitement about potential airdrops

**Significance**: First introduction of badge system, sets up distribution phases  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`

---

### Post #4: Cognitive Assessment
**Date**: October 20, 2025  
**Post ID**: `1980355518771392550`  
**Caption**: *(blank)*  
**Video Duration**: ~48 seconds  
**Video Description**:
- Black screen with "STOP" text
- Breathing instructions with countdowns ("Breathe In/Out")
- Flashes anime-style images:
  - Hospital scenes
  - Animals (rabbit, dog, tiger)
  - Forest
  - Fire
- Ends with "Cognitive Assessment Complete" text
- Badge requirements displayed:
  - 1,000 total badges
  - 250 Rabbit 🐰
  - 750 Moth 🐝
- Instructions: DM Solana address within 24 hours
- Style: Chaotic, psychological

**Requirements**: 
- 1,000 total badges (250 Rabbit 🐰, 750 Moth 🐝)
- 24-hour deadline to DM Solana wallet address

**Puzzle/Lore Hooks**: 
- Establishes "cognitive assessment" lore concept
- Psychological testing theme
- Animal imagery connects to badge types
- Time pressure (24-hour deadline)

**Community Reactions**: 
- Chaos and excitement
- People scrambling to DM wallets
- KOLs and influencers frustrated (no backdoor deals)
- Market was down, but timeline was hyped

**Significance**: First badge distribution phase begins  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`

---

### Post #5: Distribution Complete
**Date**: October 22, 2025  
**Post ID**: `1981125805104312473`  
**Caption**: *(blank)*  
**Video Duration**: ~26 seconds  
**Video Description**:
- Hands opening green and red boxes containing badges
- Computer screens showing "Distribution Complete, Field Deployment Authorized"
- Badges being pinned or dropped
- First hint at Snake 🐍 badge
- Static/aggressive sounds toward end
- Style: Simple, reveal-focused

**Distribution**:
- Phase 1: 250 Rabbit 🐰 badges airdropped first to 250 people
- Phase 2: 750 Moth 🐝 badges airdropped after Rabbits to next wave

**Puzzle Discovery**: 
- Spectrograms hidden in audio (discovered by @0xstateoftheart)
- Reveals images of:
  - Time machine
  - Testing chamber with badge on device
- Found using audio analysis tools

**Puzzle/Lore Hooks**: 
- "Field Deployment Authorized" suggests operational phase
- Snake 🐍 badge first teased
- Spectrogram puzzle rewards careful analysis

**Community Reactions**: 
- Excitement about receiving badges
- Puzzle solvers discovering spectrograms
- Community holding badges (diamond hands) despite down market

**Significance**: First two badge phases complete, Snake teased, first puzzle discovered  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`, `sources/thread-4.md`

---

### Post #6: Snake Badge Distribution Begins
**Date**: October 24, 2025  
**Post ID**: `1981740247504269582`  
**Caption**: "🐰 = 2 🐍 🐝 = 1 🐍 monday"  
**Video Duration**: ~45 seconds  
**Video Description**:
- Old computers transmitting protocols (Phase 3)
- Printers outputting papers
- Logos appearing
- TV channels flipping through scenes:
  - Traffic jam airdrop
  - Concert
  - Snake handler
  - News: "Breaking: Research Facility Attacked"
  - Kitchen fire
  - Bedroom with jacket
  - Hand picking up phone
- Style: Broadcast glitch, chaotic

**Distribution Mechanics**:
- Rabbit 🐰 holders receive 2 Snake 🐍 badges each
- Moth 🐝 holders receive 1 Snake 🐍 badge each
- Holders decide: Hold, Sell, or Give to people they thought would benefit the community

**Puzzle Elements**: 
- Channel numbers visible (potential codes)
- Background chatter and dialogue fragments
- Hidden numerical codes
- "Research Facility Attacked" news segment
- Various scenes may contain clues

**Puzzle/Lore Hooks**: 
- Decentralized distribution model (community-led)
- Facility attack news connects to lore
- Channel numbers suggest puzzle layer
- Multiple visual elements to analyze

**Community Reactions**: 
- Panic and excitement about community-led distribution
- Skepticism about how it would work
- Concerns about greed in down market
- Eventually: community self-organized successfully

**Significance**: Major social experiment begins, community takes control  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`, `sources/thread-7.md`

---

### Post #7: Puzzle Escalation - Channel Flipping
**Date**: October 27, 2025  
**Post ID**: `1982894878280569063`  
**Caption**: "dropping to distributors stage 1: complete stage 2: initiating"  
**Video Duration**: ~62 seconds  
**Video Description**:
- More TV channel flipping:
  - Highway scene
  - Jungle snake
  - Taxi argument in city at night
  - News broadcasts
  - Record player in bookshelf
  - Kitchen stove fire
  - Dark bedroom with black jacket on bed
  - Hand dialing phone
- Ends with person in dark room picking up item
- Style: Puzzle-heavy, narrative-building

**Puzzle Discovery**: 
- **Community Shoutouts** (hidden in video):
  - @TheKeeperOfX: Radios with "HEMLET CORP" branding (meme reference: "the helmet stays on")
  - @CharliegNFTeth: Spilled "Frog Gummies" in car console (ribbit meme)
  - @satoshi_owl: Owl hoots in background audio
- **Hidden Contrast Codes**: 
  - Visible only when adjusting image contrast
  - Deciphered to: HOOTS-49, HOPS-13, STAYS-28
  - These codes were given to the three community members
  - Combined to form PIN for neuko.ai website unlock

**Puzzle/Lore Hooks**: 
- Most complex puzzle to date
- Multiple layers: visual, audio, community involvement
- Leads directly to website access
- Team involved community members in puzzle distribution

**Community Reactions**: 
- Intense puzzle-solving effort
- Community members writing down channel numbers
- Analyzing walls, backgrounds, audio
- @thrax_nft and @eleven88eth key solvers
- PTSD jokes about unpacking the video

**Significance**: Leads to neuko.ai website unlock, demonstrates team watching community  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`, `sources/thread-4.md`, `sources/thread-5.md`, `sources/thread-7.md`

---

### Post #8: "Last Call" Ultimatum
**Date**: October 31, 2025 (Halloween)  
**Post ID**: `1984362507784351883`  
**Caption**: "last call, 🐍 hoarders"  
**Video Duration**: ~41 seconds  
**Video Description**:
- Aerial shots of forested industrial facility
- Black/orange buildings with hexagonal logo flag
- People in tactical gear with walkie-talkies
- Rural roads at sunset
- Car interior with snacks (Frog Gummies)
- Wire fences being cut
- Large hangar with stealth bombers and control consoles
- Groups of uniformed personnel in briefing room
- Elevator labeled "SF-06"
- Rotating mechanical device
- Voiceover in radio communication style

**Video Transcript**:
- "Good morning, operatives. I've been with the paperwork here and I take pleasure in letting you know that the deployment of snake badges through your work in this field has mostly been successful"
- "Looks like we got some good new-"
- "Rabbits & Moth did not yet recruit. You have 24 hours to do so"
- "And those who failed to follow protocol entirely, well obviously Hydrat D is flagged"
- "After desertion G304 and damages facility suffered, we'll be moving all operations underground for a little while"
- "Let's lock this shit down"

**Message**: 
- Rabbit 🐰 and Moth 🐝 holders who haven't distributed Snake 🐍 badges given 24-hour ultimatum
- Consequences for non-compliance hinted but not revealed

**Puzzle Elements**: 
- Hidden contrast codes in shoutout scenes (same as Post #7)
- HEMLET CORP branding (Keeper shoutout)
- Frog Gummies (Charlie shoutout)
- Owl hoots in audio (satoshi_owl shoutout)
- "SF-06" door - unknown significance
- Stealth bomber - potential airdrop reference?

**Lore Elements**: 
- "G304" introduced (G*Boy designation)
- Facility attacks referenced
- "Desertion" mentioned
- Operations moving underground
- MITER-Corp theme (play on real-world MITRE)

**Community Reactions**: 
- Theories about consequences
- Puzzle solvers analyzing every frame
- Community members creating debrief threads
- Speculation about flagged operatives

**Status**: Consequences of ultimatum still unknown  
**Source**: `sources/info-dump.md`, `sources/thread-5.md`

---

### Post #9: Website Portal Reveal
**Date**: November 5, 2025  
**Post ID**: `1986152753412919577`  
**Caption**: "https://www.neuko.ai/"  
**Video Duration**: ~60 seconds  
**Video Description**:
- Montage of old white TV sets
- Each screen displays animated hooded figures (red cape, black eyes) in fantastical scenes:
  - Desert landscapes
  - Skateboarding
  - Bubble scenes
  - Party scenes
  - Mountain summit
  - Fire
  - Bedroom
  - Throne made of TVs
  - Dancing
  - Heart explosion
- No text overlays or subtitles
- Style: Montage, creative explosion, throwback to first video

**Website**: 
- Minimalistic design with "NEUKO" title
- Logo: three yellow figures on red background
- TV frame aesthetic
- "Unlock" section with "Enter PIN to unlock" prompt
- PIN system requires codes from previous puzzles
- Console logs accessible via browser dev tools (discovered by @thrax_nft)

**Treasure Trail**: 
- Post-video treasure trail initiated
- For wallet holding 3x badges (one of each type)
- Community puzzle hunt

**Puzzle/Lore Hooks**: 
- Website officially revealed
- PIN unlock system (codes from Post #7)
- Console logs contain G*Boy directives and lore
- Treasure trail adds new puzzle layer
- Visual throwback to launch video

**Community Reactions**: 
- Excitement about website access
- Puzzle solvers extracting console logs
- Treasure trail hunters active
- Appreciation for visual creativity

**Significance**: Website officially revealed, new puzzle layer unlocked  
**Source**: `sources/info-dump.md`, `sources/thread-3.md`, `sources/thread-7.md`

---

### Post #10: "Catch Up, Sneeb" - Claymation Lore Dump
**Date**: November 12, 2025  
**Post ID**: `1988703965694111829`  
**Caption**: "catch up, sneeb"  
**Video Duration**: ~124 seconds  
**Video Description**:
- Stop-motion clay/toy figures in studio setting
- Figures discuss conspiracies and experiments
- Scenes include:
  - Lab experiments
  - Model facility
  - Graffiti
  - Toy-like visuals
- Humorous yet eerie dialogue
- Style: Playful yet dark, claymation

**Video Content** (from subtitles/dialogue):
- Discussion of MITER-Corp lab experiments
- "Homeless babies" experimented on to create psychic soldiers
- Animal badges represent test subjects/operatives
- G*Boy escape and "general chaos"
- References to "darker, insidious experiments"
- Humorous lines like "darker, insidious experiments"

**Lore Reveals**:
- **MITER-Corp**: Fictional corporation conducting experiments (play on real-world MITRE)
- **Badge Symbolism**:
  - Rabbit 🐰: Early stress tests
  - Moth 🐝: Sealed units
  - Snake 🐍: Couriers
- **G*Boy/G304**: Escaped subject causing chaos
- **Experiments**: Psychic soldiers created from "homeless babies"
- **Five Core Characters**: Hints at "Neuko gang" behind scenes

**Puzzle/Lore Hooks**: 
- Major lore dump
- Recaps entire story arc
- Explains badge system in-universe
- Hints at deeper conspiracy
- May contain hidden clues (community still analyzing)

**Community Reactions**: 
- Praise for claymation quality
- Excitement about lore reveals
- Theories about five core characters
- Interpretation as real lore hints
- Some puzzles may still be hidden

**Significance**: Major lore expansion, explains badge system, recaps story  
**Source**: `sources/info-dump.md`

---

## Engagement Summary

**Overall Growth**:
- Started: 0 followers (early October 2025)
- Current: ~13,462 followers (as of November 13, 2025)
- Growth: ~10K followers in first 2 weeks
- Mindshare: 0.25% (per cookie.fun) - higher than many established projects

**Engagement Patterns**:
- Average 1K-3K likes per post
- 200-700 reposts
- 100K+ views on major videos
- 100% organic (no paid promotion)
- Broke X algorithm organically

**Community Response**:
- Strong diamond hands (2-3% listed on Magic Eden)
- Active puzzle-solving community
- Community-driven content creation
- High conviction and loyalty

## Puzzle Evolution

1. **Simple Videos** (Posts #2-3): Visual storytelling, no hidden puzzles
2. **Spectrograms** (Post #5): Audio analysis reveals hidden images
3. **Multi-Layer Puzzles** (Posts #6-7): Visual, audio, community involvement
4. **Website Integration** (Post #9): Console logs, PIN system, treasure trails
5. **Lore Dumps** (Post #10): Story expansion, may contain hidden elements

## Sources

All post details cite original source files in `sources/` directory. See individual entries for specific file references.

